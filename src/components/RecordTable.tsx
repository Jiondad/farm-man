    ...
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-xs flex flex-col h-full overflow-hidden relative">
                ...
                return (
                  <tr key={record.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-3 py-2 whitespace-nowrap text-xs text-slate-700">
                      <div className="flex items-center gap-1.5">{record.date}</div>
                    </td>
                    ...
                    <td className="px-3 py-2 whitespace-nowrap text-xs text-slate-600 text-center">
                      {record.workersCount}명 / {record.workHours}시간
                    </td>
                    <td className="px-3 py-2 text-xs text-slate-600 max-w-xs truncate">{record.content}</td>
                    <td className="px-3 py-2 text-xs text-slate-600 truncate max-w-[120px]">{record.materials || '-'}</td>
                    ...
                    <td className="px-3 py-2 whitespace-nowrap text-xs text-rose-600 text-right">
                      {Number(record.expense || 0).toLocaleString()}
                    </td>
                    <td className="px-3 py-2 whitespace-nowrap text-center relative">
                      {record.photoUrl ? (
                        <div className="inline-flex p-1.5 text-sky-600 cursor-pointer" onMouseEnter={() => setHoveredImage(record.photoUrl || null)} onMouseLeave={() => setHoveredImage(null)}>
                    ...
                    <td className="px-3 py-2 whitespace-nowrap text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => onEditRecordClick(record)} className="p-1 text-slate-400 hover:text-emerald-700"><Edit2 className="w-3 h-3" /></button>
                        <button onClick={() => onDeleteRecordClick(record.id)} className="p-1 text-slate-400 hover:text-rose-600"><Trash2 className="w-3 h-3" /></button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      {hoveredImage && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 p-2 bg-white rounded-xl shadow-2xl border border-slate-200 pointer-events-none animate-in fade-in zoom-in duration-200">
          <img src={hoveredImage} alt="현장 사진" className="max-w-[300px] max-h-[300px] object-cover rounded-lg" />
        </div>
      )}
    </div>
  );
